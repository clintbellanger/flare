...
void EnemyManager::loadGraphics(string type_id) {
	...
	sprites[gfx_count] = IMG_Load((PATH_MOD + "images/enemies/" + type_id + ".png").c_str());
...
void EnemyManager::loadSounds(string type_id) {
	...
	sound_phys[sfx_count] = Mix_LoadWAV((PATH_MOD + "soundfx/enemies/" + type_id + "_phys.ogg").c_str());
	sound_ment[sfx_count] = Mix_LoadWAV((PATH_MOD + "soundfx/enemies/" + type_id + "_ment.ogg").c_str());
	sound_hit[sfx_count] = Mix_LoadWAV((PATH_MOD + "soundfx/enemies/" + type_id + "_hit.ogg").c_str());
	sound_die[sfx_count] = Mix_LoadWAV((PATH_MOD + "soundfx/enemies/" + type_id + "_die.ogg").c_str());
	sound_critdie[sfx_count] = Mix_LoadWAV((PATH_MOD + "soundfx/enemies/" + type_id + "_critdie.ogg").c_str());
...
void EnemyManager::handleNewMap () {
	...
	while (!map->enemies.empty()) {
		...
		enemies[enemy_count]->stats.load(PATH_MOD + "enemies/" + me.type + ".txt");
		if (enemies[enemy_count]->stats.animations != "") {
			// load the animation file if specified
			enemies[enemy_count]->loadAnimations(PATH_MOD + "animations/" + enemies[enemy_count]->stats.animations + ".txt");
		}
		else {
