...
GameStatePlay *GameStatePlay::instance = NULL;

GameStatePlay::GameStatePlay(int _game_slot)
	: GameState(true)
	, powers()
	, items()
	, camp()
	...
	, pc(powers, map)
	, enemies(powers, map)
	, hazards(powers, pc, enemies)
	, menu(powers, pc.stats, camp, items)
	, loot(items, enemies, map)
	, npcs(map, loot, items)
...
void GameStatePlay::checkLoot() {
	if (inp->pressing[MAIN1] && !inp->lock[MAIN1] && pc.stats.alive) {
		...
		if (pickup.item) {
			...
			camp.setStatus(pickup.item->pickup_status);
		}
		else if (gold > 0) {
			inp->lock[MAIN1] = true;
...
void GameStatePlay::checkEquipmentChange() {
	if (menu.inv.changed_equipment) {
		...
		const MenuItemStorage &equip = menu.inv.inventory[EQUIPMENT];
...
void GameStatePlay::checkConsumable() {
	if (powers.used_item != -1) {
		const Item &item = menu.getItemManager().getItem(powers.used_item);
		if (item.type == ITEM_TYPE_CONSUMABLE) {
			menu.inv.remove(item);
			powers.used_item = -1;
		}
	}
}
...
void GameStatePlay::checkNotifications() {
    ...
    if (quests.getNewQuestNotification()) { //remove if no quests
        menu.act.requires_attention[MENU_LOG] = false;
    }
    if (quests.getNewQuestNotification()) {
