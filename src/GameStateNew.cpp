...
GameStateNew::GameStateNew() : GameState() {
	...
	// set up buttons
	button_exit.reset(new WidgetButton("images/menus/buttons/button_default.png"));
	button_exit->label = msg->get("Cancel");
	button_exit->pos.x = VIEW_W_HALF - button_exit->pos.w/2;
	button_exit->pos.y = VIEW_H - button_exit->pos.h;
	button_exit->refresh();
	
	button_create.reset(new WidgetButton("images/menus/buttons/button_default.png"));
...
void GameStateNew::loadGraphics() {
	...
	portrait_border.set_color_key(SDL_SRCCOLORKEY, portrait_border.map_rgb(255, 0, 255)); 
	portrait_border.display_format_alpha();
}

void GameStateNew::loadPortrait(const string& portrait_filename) {
	portrait_image.reset_and_load("images/portraits/" + portrait_filename + ".png");
	// Will never be true
	if (!portrait_image) return;
	
	portrait_image.display_format_alpha();
}
...
void GameStateNew::render() {
	...
	if (portrait) {
		SDL_BlitSurface(portrait_image.get(), &src, screen, &dest);		
	}
	if (portrait_border) {
